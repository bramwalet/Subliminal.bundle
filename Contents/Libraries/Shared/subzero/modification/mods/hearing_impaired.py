...
class HearingImpaired(SubtitleTextModification):
    ...
    processors = [
        ...
        NReProcessor(re.compile(r'(?sux)[([].+(?=[À-Ž]{3,}).+[)\]]'), "", name="HI_brackets"),
        ...
        NReProcessor(re.compile(r'(?u)(^(?=.*[À-Ž]{4,})[À-Ž-_\s]+:[^0-9]\s*)'), "", name="HI_before_colon"),

        # all caps line (at least 4 consecutive uppercase chars)
        NReProcessor(re.compile(r'(?u)(^(?=.*[À-Ž]{4,})[À-Ž-_\s]+$)'), "", name="HI_all_caps"),
